...
@app.route('/job_parse/count')
def job_parse_count():
    description = request.args.get('job_desc')
    job_words = {}
    omit_words = ['and', 'to', 'of', 'in', 'the', 'with', 'or', 'and/or', 'i.e.', 'etc.', 'as', 'by', 'for', 'on', 'not', 'but', 'is']
...
@app.route('/job_parse/resume-comp')
def resume_comp():
    resume_words = []
    job_words = []
    missing_words = []
    ...
    resume_count = request.args.get('resume')
